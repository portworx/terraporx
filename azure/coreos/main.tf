...
resource "azurerm_template_deployment" "dcostmpl" {
    name = "dcostmpl-01"
    ...
    },
    {
      ...
      "properties": {
        ...
        "storageProfile": {
          ...
          "dataDisks": [
            {
              "name" : "[concat(variables('vmNamePrefix'),'-dataDisk0')]",
              "diskSizeGB": "[parameters('sizeOfEachDataDiskInGB')]",
              "lun": 0,
              "createOption": "Empty",
              "vhd": {
                 "uri": "[concat('http://',parameters('newStorageAccountName'),'.blob.core.windows.net/vhds/',variables('vmNamePrefix'), '-dataDisk',copyindex(),'.vhd')]"
               }
          }],
          "osDisk": {
            "name": "[concat(variables('vmNamePrefix'), copyindex())]",
...
DEPLOY
      "parameters" =  {
          "newStorageAccountName" = "${var.newStorageAccountName}"
          "vmSize" = "${var.vmSize}"
          "adminUsername" = "${var.adminUsername}"
          "sshKeyData" = "${var.sshKeyData}"
          "discoveryUrl" = "${var.discoveryUrl}"
          "sizeOfEachDataDiskInGB" = "${var.sizeOfEachDataDiskInGB}"
       }
